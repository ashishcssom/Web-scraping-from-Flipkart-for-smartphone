...
class Flipkart():
    ...
    def __init__(self,page):
        ...
        self.url = 'https://www.flipkart.com/search?q=smartphone&as=on&as-show=on&otracker=AS_Query_\
            HistoryAutoSuggest_1_3_na_na_na&otracker1=AS_Query_HistoryAutoSuggest_1_3_na_na_na&as-pos=1&as-\
                type=HISTORY&suggestionId=smartphone&requestId=d9267ef4-0ed1-4f13-9e71-a24325e441d4&as-searchtext=sma&page='+str(page)
    ...
    def create_csv_file(self):
        ...
        rowHeaders = ["Name", "Storage_details", "Screen_size", "Camera_details", "Battery_details", "Processor", "Warranty", "Price in Rupees","Rating","Actual price","%Off"]
        self.file_csv = open('Flipkart_output'+str(self.page)+'.csv', 'w', newline='', encoding='utf-8')
    ...
    def data_scrap(self):
        ...
        for i in first_page_mobiles:
            ...
            try:
                ...
                Rating = i.find('div', class_='hGSR34').get_text()
                Actual_price=i.find('div', class_='_3auQ3N _2GcJzG').get_text().replace('â‚¹','')
                Off=i.find('div', class_='VGWI6T').get_text()
            except:
                warranty_details = "No data available"
                Rating = "No data available"
                Actual_price="No data available"
                Off="No data available"
            price = price.text[1:]
            self.mycsv.writerow({"Name": Name, "Storage_details": storage, "Screen_size": screen_size, "Camera_details": camera_details, "Battery_details": battery_details, "Processor": processor, "Warranty": warranty_details, "Price in Rupees": price,"Rating":Rating,"Actual price":Actual_price,"%Off":Off})
